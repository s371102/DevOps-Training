...
# Create a null_resource to handle provisioners with dynamic references
resource "null_resource" "puppet_provisioner" {
  triggers = {
    instance_ip = openstack_compute_instance_v2.puppet_master.access_ip_v4
  }
  ...
  connection {
    ...
    provisioner "file" {
      source = "/home/ubuntu/puppet_project/manifest.pp" # This file contains the hping3 to be install in the system
...
output "puppet_master_ip" {
  value = openstack_compute_instance_v2.puppet_master.access_ip_v4 # This will print the IP address of the created VM
}